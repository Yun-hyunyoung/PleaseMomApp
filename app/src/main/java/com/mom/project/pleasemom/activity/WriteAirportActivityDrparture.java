 ...
 */
public class WriteAirportActivityDrparture extends Activity {
    AutoCompleteTextView autoTextView;
    TextView returnTextView;
    AutoAirportNameAdapter adapter;
    ArrayList<AirportDTO> airportDTOs;
    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_write_airport_departure);
        loadAirportInfo();

        Button btn = (Button)findViewById(R.id.btnFindAirport);
        autoTextView  = (AutoCompleteTextView)findViewById(R.id.autoFindAirportDeparture);
        autoTextView.setThreshold(1);

        returnTextView = BoardWriteActivity.txtDeparture;
        adapter = new AutoAirportNameAdapter(this, R.layout.autocomplete_airport_name, airportDTOs);
        autoTextView.setAdapter(adapter);

        btn.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                returnTextView.setText(autoTextView.getText().toString());
                finish();
            }
        });
    }
